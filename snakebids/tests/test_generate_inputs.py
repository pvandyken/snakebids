...
from snakebids.tests import strategies as stx
...
class TestBidsInputEq:
    @given(stx.bids_input(), stx.everything_except(BidsComponent))
...
class TestBidsProperties:
    ...
    def test_input_lists_derives_from_zip_lists(
        ...
        input_lists: Dict[str, List[str]] = data.draw(stx.bids_input_lists(min_size))
        ...
        path = stx.get_bids_path(zip_lists)
    ...
    @given(st.dictionaries(stx.bids_entity(), stx.bids_value(), min_size=1))
